...
// ================= Custom Payment Process Functions ================
let paymentChoice = 0;

// Define a function that shows the popup div card
function showPopup() {
    const popup = document.getElementById('popup-payment');
    popup.style.display = 'block';
}

// Define a function that processes the payment and hides the popup div card
async function processPayment(item_id) {
    const phone = document.getElementById('phone').value;
    const user_name = document.getElementById('name_input').value;
    const user_email = document.getElementById('email').value;
    const url = 'https://656962e5a1001a19359c.appwrite.global/';
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    const loader = document.getElementById('processing_payment');
    const phone_details = document.getElementById('phone_details');

    // Validating mail before continueing
    if (!emailRegex.test(user_email)) {
        alert('Please enter a valid email address');
        return;
    }
    phone_details.classList.add('phone_details_disable');
    loader.classList.add('processing_payment_active');
    //
    const payment_success_pop = document.getElementById('popup_success_payment');
    const h2Title = payment_success_pop.querySelector('h2');
    const content_message = payment_success_pop.querySelector('p');
    const image_content = payment_success_pop.querySelector('img');
    const ok_button = payment_success_pop.querySelector('button');
        ...
        if (xhr.readyState === XMLHttpRequest.DONE) {
            ...
            if (xhr.status === 200) {
                ...
                if (data.message === 'user found') {
                    if (data.user_info.ResultCode === 0) {
                        image_content.src = './images/tick.png';
                        h2Title.innerText = 'Payment Success';
                        content_message.innerText = data.user_info.ResultDesc;
                        ok_button.style.backgroundColor = '#6fd649';
                        closePopup();
                        loader.classList.remove('processing_payment_active');
                        phone_details.classList.remove('phone_details_disable');
                        payment_success_pop.classList.add('open-popup_success_payment');
                    } else {
                        image_content.src = './images/warning.png';
                        h2Title.innerText = 'An Error Occured!!';
                        content_message.innerText = data.user_info.ResultDesc;
                        ok_button.style.backgroundColor = '#c92228';
                        closePopup();
                        loader.classList.remove('processing_payment_active');
                        phone_details.classList.remove('phone_details_disable');
                        payment_success_pop.classList.add('open-popup_success_payment');
                ...
                // Throw an error with the status and the status text
...
function findActivePaymentMethod1() {
    paymentChoice = 0;
}
function findActivePaymentMethod2() {
    paymentChoice = 1;
}

function closeSuccessPopup() {
    const pop_success = document.getElementById('popup_success_payment');
    pop_success.classList.remove('open-popup_success_payment');
}
...
const setUserName = () => {
    ...
    const login_logout_btn = document.getElementById('login_logout_tag');
    if (user_data) {
        user_data = JSON.parse(user_data);
        login_logout_btn.innerHTML = `Log Out</strong><i class="bi bi-chevron-right"></i>`;
        const userNameElement = document.getElementById('user_name_element');
        if (userNameElement) {
            userNameElement.innerText = user_data.name;
        }
    } else {
        login_logout_btn.innerHTML = `Log In</strong><i class="bi bi-chevron-right"></i>`;
        const userNameElement = document.getElementById('user_name_element');
        if (userNameElement) {
